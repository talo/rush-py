...
The data for the exported values are present, and the descriptor grid coordinates are given as well (transposed, so the first array contains the x coordinates, the second the y coordinates, and the third the z coordinates). Descriptor grid weights are also present. TODO: Learn when these weights are something other than 1.0, and explain how to create these types of grids, how they behave, and what they are useful for.
...
Note that in the case of fragmentation, the data are stored on a per-nmer basis, and if both descriptors are requested, they will be stored together at the innermost level of nesting in the HDF5 file. When exported to JSON, the different descriptors are stored in entirely separate outer keys in the JSON file, making it easy to work with just one or the other. Otherwise, the structure of the HDF5 file is preserved in the JSON file.

### Other Types of Descriptor Grids

In addition to `CustomDescriptorGrid`, which can be constructed by passing a single list with the x, y, and z coordinates of each point in the grid listed sequentially, i.e. `[x1, y1, z1, x2, y2, z2, ...]`, a descriptor grid can be constructed via `StandardDescriptorGrid` or `DescriptorGrid`. 

There are two valid values of `StandardDescriptorGrid`: `StandardDescriptorGrid("SG1")` and `StandardDescriptorGrid("SG2")`. TODO: Learn these grids' behavior and document them. They seem to be very fine grids and construct many points.
